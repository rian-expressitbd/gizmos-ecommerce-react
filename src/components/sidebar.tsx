...
export const Sidebar = ({ className }: SidebarProps) => {
  ...
  const sidebarContent = (
    <ul className='space-y-2 max-h-[80vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100 px-2'>
      {menuItems.map((item) => (
        <li key={item.title}>
          {item.submenu ? (
            <>
  ...
  return (
    <AnimatePresence>
      <aside className='bg-white dark:bg-gray-700'>
        {isDesktop ? (
          <motion.div
            key='sidebar-desktop'
